...
public class WalletOperationService extends BusinessService<WalletOperation> {
	...
	public BigDecimal getRatedAmount(Seller seller, Customer customer, CustomerAccount customerAccount,
			BillingAccount billingAccount, UserAccount userAccount, Date startDate, Date endDate,
	 ...
	 */
	public BigDecimal getBalanceAmount(Seller seller, Customer customer, CustomerAccount customerAccount,
			BillingAccount billingAccount, UserAccount userAccount, Date startDate, Date endDate, boolean amountWithTax,
			int mode) {
		...
		try {
			...
			strQuery.append("select SUM(r." + (amountWithTax ? "amountWithTax" : "amountWithoutTax") + ") from "
					+ WalletOperation.class.getSimpleName() + " r " + "WHERE 1=1 ");
	...
	public WalletOperation rateOneShotApplication(Subscription subscription, OneShotChargeInstance chargeInstance,
			BigDecimal inputQuantity, BigDecimal quantityInChargeUnits, Date applicationDate, boolean isVirtual,
		...
		if (invoiceSubCategory == null) {
			throw new IncorrectChargeTemplateException(
					"InvoiceSubCategory is null for chargeTemplate code=" + chargeTemplate.getCode());
		}
		...
		InvoiceSubcategoryCountry invoiceSubcategoryCountry = invoiceSubCategoryCountryService
				.findInvoiceSubCategoryCountry(invoiceSubCategory.getId(), countryId, applicationDate);
		...
		if (tax == null) {
			tax = invoiceSubCategoryService.evaluateTaxCodeEL(invoiceSubcategoryCountry.getTaxCodeEL(),
					chargeInstance.getUserAccount(), chargeInstance.getUserAccount().getBillingAccount(), null);
			if (tax == null) {
				throw new IncorrectChargeTemplateException(
						"No tax exists for invoiceSubcategoryCountry id=" + invoiceSubcategoryCountry.getId());
			}
		}

		WalletOperation chargeApplication = chargeApplicationRatingService.rateChargeApplication(chargeInstance,
				ApplicationTypeEnum.PUNCTUAL, applicationDate, chargeInstance.getAmountWithoutTax(),
				chargeInstance.getAmountWithTax(), inputQuantity, quantityInChargeUnits, currency, countryId,
				...
				orderNumberOverride != null
						? (orderNumberOverride.equals(ChargeInstance.NO_ORDER_NUMBER) ? null : orderNumberOverride)
						: chargeInstance.getOrderNumber(),
				null, null, null, false, isVirtual);

		return chargeApplication;
	}

	public WalletOperation oneShotWalletOperation(Subscription subscription, OneShotChargeInstance chargeInstance,
			BigDecimal inputQuantity, BigDecimal quantityInChargeUnits, Date applicationDate, boolean isVirtual,
			String orderNumberOverride) throws BusinessException {
		...
		log.debug(
				...
				new Object[] { subscription.getId(), quantityInChargeUnits,
						chargeInstance.getChargeTemplate().getUnitMultiplicator(), applicationDate,
						chargeInstance.getId(), chargeInstance.getDescription() });

		WalletOperation walletOperation = rateOneShotApplication(subscription, chargeInstance, inputQuantity,
				quantityInChargeUnits, applicationDate, isVirtual, orderNumberOverride);
		ChargeTemplate chargeTemplate = chargeInstance.getChargeTemplate();
		...
		Boolean immediateInvoicing = (oneShotChargeTemplate != null
				&& oneShotChargeTemplate.getImmediateInvoicing() != null)
						? oneShotChargeTemplate.getImmediateInvoicing() : false;
	...
	public WalletOperation rateProductApplication(ProductChargeInstance chargeInstance, boolean isVirtual)
		...
		Long countryId = country.getId();
		InvoiceSubcategoryCountry invoiceSubcategoryCountry = invoiceSubCategoryCountryService
				.findInvoiceSubCategoryCountry(invoiceSubCategory.getId(), countryId, chargeInstance.getChargeDate());
		...
		WalletOperation chargeApplication = chargeApplicationRatingService.rateChargeApplication(chargeInstance,
				ApplicationTypeEnum.PUNCTUAL, chargeInstance.getChargeDate(), chargeInstance.getAmountWithoutTax(),
				chargeInstance.getAmountWithTax(), chargeInstance.getQuantity(), null, currency, countryId,
				tax.getPercent(), null, null, invoiceSubCategory, chargeInstance.getCriteria1(),
				chargeInstance.getCriteria2(), chargeInstance.getCriteria3(), chargeInstance.getOrderNumber(), null,
	...
	// rateProductApplicationVirtual(ProductChargeTemplate chargeTemplate,
	// UserAccount userAccount, String offerCode, BigDecimal inputQuantity,
	// BigDecimal quantity, Date applicationDate, BigDecimal amountWithoutTax,
	// BigDecimal amountWithTax, String criteria1, String criteria2, String
	// criteria3)
	// throws BusinessException {
	//
	...
	// if (invoiceSubCategory == null) {
	// throw new IncorrectChargeTemplateException("InvoiceSubCategory is null
	// for chargeTemplate code=" + chargeTemplate.getCode());
	// }
	//
	// TradingCurrency currency =
	// userAccount.getBillingAccount().getCustomerAccount().getTradingCurrency();
	// if (currency == null) {
	// throw new IncorrectChargeTemplateException("No currency exists for
	// customerAccount id=" +
	// userAccount.getBillingAccount().getCustomerAccount().getId());
	// }
	//
	// TradingCountry country =
	// userAccount.getBillingAccount().getTradingCountry();
	// if (country == null) {
	// throw new IncorrectChargeTemplateException("No country exists for
	// billingAccount id=" + userAccount.getBillingAccount().getId());
	// }
	//
	// Long countryId = country.getId();
	// InvoiceSubcategoryCountry invoiceSubcategoryCountry =
	// invoiceSubCategoryCountryService.findInvoiceSubCategoryCountry(invoiceSubCategory.getId(),
	// countryId);
	//
	// if (invoiceSubcategoryCountry == null) {
	// throw new IncorrectChargeTemplateException("No invoiceSubcategoryCountry
	// exists for invoiceSubCategory code=" + invoiceSubCategory.getCode() + "
	// and trading country="
	...
	// Tax tax = invoiceSubcategoryCountry.getTax();
	// if (tax == null) {
	// throw new IncorrectChargeTemplateException("No tax exists for
	// invoiceSubcategoryCountry id=" + invoiceSubcategoryCountry.getId());
	// }
	//
	// WalletOperation chargeApplication =
	// chargeApplicationRatingService.rateChargeApplicationVirtual(chargeTemplate,
	// userAccount, offerCode, null, ApplicationTypeEnum.PUNCTUAL,
	// applicationDate, null, null, inputQuantity, quantity, currency,
	// countryId, tax.getPercent(), null, null, invoiceSubCategory, criteria1,
	// criteria2, criteria3, null,
	// null, null, false);
	//
	// return chargeApplication;
	// }

	// Be careful to use this method only for the first application of a
	// recurring charge
	...
	public WalletOperation rateSubscription(Date subscriptionDate, RecurringChargeInstance chargeInstance,
			Date nextapplicationDate, boolean preRateOnly) throws BusinessException {
		...
		log.debug(
				"rateSubscription subscriptionDate={} applicationDate={}, nextapplicationDate={},previousapplicationDate={}",
				subscriptionDate, applicationDate, nextapplicationDate, previousapplicationDate);
		...
		if (invoiceSubCategory == null) {
			throw new IncorrectChargeTemplateException(
					"invoiceSubCategory is null for chargeTemplate code=" + recurringChargeTemplate.getCode());
		}
		...
		if (!preRateOnly) {
			result = chargeApplicationRatingService.rateChargeApplication(chargeInstance,
					...
					chargeInstance.getAmountWithTax(), inputQuantity, null, currency, countryId, tax.getPercent(), null,
					nextapplicationDate, recurringChargeTemplate.getInvoiceSubCategory(), chargeInstance.getCriteria1(),
			...
			String languageCode = chargeInstance.getUserAccount().getBillingAccount().getTradingLanguage().getLanguage()
					.getLanguageCode();
			result = chargeApplicationRatingService.prerateChargeApplication(chargeInstance.getChargeTemplate(),
					...
					chargeInstance.getAmountWithoutTax(), chargeInstance.getAmountWithTax(), inputQuantity, null,
					currency, countryId, languageCode, tax.getPercent(), null, nextapplicationDate,
					recurringChargeTemplate.getInvoiceSubCategory(), chargeInstance.getCriteria1(),
					chargeInstance.getCriteria2(), chargeInstance.getCriteria3(), chargeInstance.getOrderNumber(),
					applicationDate, DateUtils.addDaysToDate(nextapplicationDate, -1), null,
	...
	public void chargeSubscription(RecurringChargeInstance chargeInstance) throws BusinessException {
		...
		log.debug(
				"ChargeApplicationService.chargeSubscription subscriptionCode={}, chargeCode={}, quantity={}, applicationDate={},chargeInstance.getId={}",
				new Object[] { serviceInstance.getSubscription().getCode(), chargeInstance.getCode(),
						chargeInstance.getQuantity(), chargeInstance.getSubscriptionDate(), chargeInstance.getId() });

		RecurringChargeTemplate recurringChargeTemplate = chargeInstance.getRecurringChargeTemplate();
		Date nextapplicationDate = getNextApplicationDate(chargeInstance);

		// if (!isChargeMatch(chargeInstance,
		// chargeInstance.getRecurringChargeTemplate().getFilterExpression())) {
		// log.debug("IPIEL: not rating chargeInstance with code={}, filter
		// expression not evaluated to true", chargeInstance.getCode());
		...
		if (recurringChargeTemplate.getApplyInAdvance() != null && recurringChargeTemplate.getApplyInAdvance()) {
			WalletOperation chargeApplication = rateSubscription(chargeInstance.getSubscriptionDate(), chargeInstance,
					nextapplicationDate);
	...
	//
	// ServiceInstance serviceInstance = chargeInstance.getServiceInstance();
	// log.debug("ChargeApplicationService.chargeSubscription
	// subscriptionCode={}, chargeCode={}, quantity={},
	// applicationDate={},chargeInstance.getId={}",
	// new Object[] { serviceInstance.getSubscription().getCode(),
	// chargeInstance.getCode(), chargeInstance.getQuantity(),
	...
	// }
	//
	// if (recurringChargeTemplate.getApplyInAdvance() != null &&
	// recurringChargeTemplate.getApplyInAdvance()) {
	// WalletOperation chargeApplication =
	// rateSubscription(chargeInstance.getSubscriptionDate(), chargeInstance,
	// nextapplicationDate);
	// // create(chargeApplication);
	// chargeWalletOperation(chargeApplication);
	// chargeInstance.setNextChargeDate(nextapplicationDate);
	...
	public void applyReimbursment(RecurringChargeInstance chargeInstance) throws BusinessException {
		...
		log.debug(
				...
				serviceInstance.getSubscription().getCode(), chargeInstance.getCode(), chargeInstance.getQuantity(),
		...
		if (recurringChargeTemplate.getCalendar() == null) {
			throw new IncorrectChargeTemplateException(
					"Recurring charge template has no calendar: code=" + recurringChargeTemplate.getCode());
		}
		Calendar cal = recurringChargeTemplate.getCalendar();
		...
		if (!isChargeMatch(chargeInstance, chargeInstance.getRecurringChargeTemplate().getFilterExpression())) {
			log.debug("IPIEL: not rating chargeInstance with code={}, filter expression not evaluated to true",
		...
		if (recurringChargeTemplate.getApplyInAdvance() != null && recurringChargeTemplate.getApplyInAdvance()) {
			...
			log.debug("reimbursment-NotApplyInAdvance applicationDate={}, nextapplicationDate={},nextChargeDate={}",
					applicationDate, nextapplicationDate, nextChargeDate);

			if (nextChargeDate != null && nextChargeDate.getTime() > nextapplicationDate.getTime()) {
				applyNotAppliedinAdvanceReccuringCharge(chargeInstance, true, recurringChargeTemplate, null);
			}
		}
	}
	 ...
	 *             business exeption.
	 */
	public List<WalletOperation> applyReccuringCharge(RecurringChargeInstance chargeInstance, boolean reimbursement,
			RecurringChargeTemplate recurringChargeTemplate, boolean forSchedule) throws BusinessException {
		...
		Calendar cal = recurringChargeTemplate.getCalendar();
		cal.setInitDate(serviceInstance.getSubscriptionDate());
		Date nextApplicationDate = reimbursement ? chargeInstance.getNextChargeDate()
				: cal.nextCalendarDate(applicationDate);
		...
		while (nextApplicationDate != null && applicationDate.getTime() < nextApplicationDate.getTime()) {
			...
			WalletOperation walletOperation = chargeApplicationRatingService.rateChargeApplication(chargeInstance,
					...
					applicationDate, chargeInstance.getAmountWithoutTax(), chargeInstance.getAmountWithTax(),
					inputQuantity, null, currency, countryId, tax.getPercent(), null, nextapplicationDate,
					invoiceSubCategory, chargeInstance.getCriteria1(), chargeInstance.getCriteria2(),
	...
	/**
	 * Create wallet operations for a recurring charges between given dates for
	 * Virtual operation.
	 * 
	 * @param chargeInstance
	 ...
	 *             business exception.
	 */
	public List<WalletOperation> applyReccuringChargeVirtual(RecurringChargeInstance chargeInstance, Date fromDate,
		...
		if (invoiceSubCategory == null) {
			throw new IncorrectChargeTemplateException("invoiceSubCategory is null for chargeTemplate code="
					+ chargeInstance.getRecurringChargeTemplate().getCode());
		}
		...
		while (applicationDate.getTime() < endApplicationDate.getTime()) {
			...
			log.debug("ApplyReccuringChargeVirtual : nextapplicationDate={}, param2={} -> quantity={}",
					nextapplicationDate, param2, inputQuantity);

			WalletOperation walletOperation = chargeApplicationRatingService.rateChargeApplication(chargeInstance,
					ApplicationTypeEnum.RECURRENT, applicationDate, chargeInstance.getAmountWithoutTax(),
					chargeInstance.getAmountWithTax(), inputQuantity, null, currency, countryId, tax.getPercent(), null,
					nextapplicationDate, invoiceSubCategory, chargeInstance.getCriteria1(),
					chargeInstance.getCriteria2(), chargeInstance.getCriteria3(), chargeInstance.getOrderNumber(),
					applicationDate, DateUtils.addDaysToDate(nextapplicationDate, -1),
					ChargeApplicationModeEnum.SUBSCRIPTION, false, true);
	...
	public void applyNotAppliedinAdvanceReccuringCharge(RecurringChargeInstance chargeInstance, boolean reimbursement,
			RecurringChargeTemplate recurringChargeTemplate, Date terminationDate) throws BusinessException {
		...
		if (tax == null) {
			...
			if (tax == null) {
				throw new IncorrectChargeTemplateException(
						"Tax is null for invoiceSubCategoryCountry id=" + invoiceSubcategoryCountry.getId());
			}
		}

		List<InvoiceSubcategoryCountry> invoiceSubcategoryCountries = invoiceSubCategoryCountryService.
					findInvoiceSubCategoryCountries (invoiceSubCategory.getId(), countryId, applicationDate);
		
		BigDecimal totalTaxPercent = new BigDecimal(0);
		for (InvoiceSubcategoryCountry invSubcategoryCountry : invoiceSubcategoryCountries)
		{
			totalTaxPercent = totalTaxPercent.add(invSubcategoryCountry.getTax().getPercent()); 
		}
		
		Date chargeDateStart = chargeInstance.getChargeDate();
		Date chargeDateEnd = reimbursement ? chargeInstance.getChargeDate() : chargeInstance.getNextChargeDate();

		cal.setInitDate(chargeInstance.getServiceInstance().getSubscriptionDate());
		Date  periodStart = DateUtils.setTimeToZero(cal.previousPeriodEndDate(chargeDateStart));
		Date periodEnd = DateUtils.setTimeToZero(cal.nextPeriodStartDate(chargeDateStart));
		
		chargeInstance.getUserAccount().getBillingAccount().getBillingCycle().getCalendar()
				.setInitDate(chargeInstance.getServiceInstance().getSubscriptionDate());
		Date maxChargeDate = chargeInstance.getUserAccount().getBillingAccount().getBillingCycle().getCalendar()
				.nextCalendarDate(chargeDateStart);

		int durationTermInMonth = 0;
		boolean isLastChargingApplied = false;
		boolean isLastChargingWillBeApplied = false;
		Date durationTermInMonthDate = null;
		if (chargeInstance.getRecurringChargeTemplate().getDurationTermInMonth() != null) {
			durationTermInMonth = chargeInstance.getRecurringChargeTemplate().getDurationTermInMonth();
			if (durationTermInMonth > 0) {
				durationTermInMonthDate = DateUtils
						.addMonthsToDate(chargeInstance.getServiceInstance().getSubscriptionDate(), durationTermInMonth);
				Integer agreementExtensionDays = chargeInstance.getServiceInstance().getSubscription()
				...
				if (chargeDateEnd != null && (chargeDateEnd.after(durationTermInMonthDate) ||  chargeDateEnd.equals(durationTermInMonthDate))) {
					chargeDateEnd = durationTermInMonthDate;
					maxChargeDate = durationTermInMonthDate;
					isLastChargingWillBeApplied = true;
				}
			}
		}

		if (terminationDate != null && terminationDate.before(chargeDateEnd)){
			chargeDateEnd = terminationDate;
			maxChargeDate = terminationDate;
			isLastChargingWillBeApplied = true;
		}
		
		CalendarJoin calendarJoin = new CalendarJoin();
		calendarJoin.setJoinCalendar2(recurringChargeTemplate.getCalendar());
		calendarJoin.setJoinCalendar1(chargeInstance.getUserAccount().getBillingAccount().getBillingCycle().getCalendar());
		calendarJoin.setJoinType(CalendarJoinTypeEnum.INTERSECT);
		
		while (chargeDateEnd != null && chargeDateStart != null && periodStart != null & periodEnd != null
				&& chargeDateStart.getTime() < maxChargeDate.getTime()) {

			if (isLastChargingWillBeApplied){
				isLastChargingApplied = true;
			}
			...
			if (recurringChargeTemplate.getSubscriptionProrata()) {
				...
				double part1 = DateUtils.daysBetween(chargeDateStart, chargeDateEnd);
				double part2 = DateUtils.daysBetween(periodStart, periodEnd);

				if (part2 > 0) {
					prorataRatio = part1 / part2;
				} else {
					log.error(
							"ApplyNotAppliedinAdvanceReccuringCharge Error in calendar dates : periodStart={}, periodEnd={}",
			...
			WalletOperation walletOperation = chargeApplicationRatingService.rateChargeApplication(chargeInstance,
					reimbursement ? ApplicationTypeEnum.PRORATA_TERMINATION : applicationTypeEnum, chargeDateStart,
					chargeInstance.getAmountWithoutTax(), chargeInstance.getAmountWithTax(), inputQuantity, null,
					currency, countryId, totalTaxPercent, null, chargeDateEnd, invoiceSubCategory,
					chargeInstance.getCriteria1(), chargeInstance.getCriteria2(), chargeInstance.getCriteria3(),
					chargeInstance.getOrderNumber(), chargeDateStart, DateUtils.addDaysToDate(chargeDateEnd, -1),
					reimbursement ? ChargeApplicationModeEnum.REIMBURSMENT : ChargeApplicationModeEnum.SUBSCRIPTION,
					false, false);
			...
			chargeDateStart = chargeDateEnd;
			calendarJoin.setInitDate(chargeInstance.getServiceInstance().getSubscriptionDate());
			chargeDateEnd = calendarJoin.nextCalendarDate(chargeDateStart);
			if (durationTermInMonthDate != null) {
				if (chargeDateEnd.after(durationTermInMonthDate)) {
					chargeDateEnd = durationTermInMonthDate;
					isLastChargingWillBeApplied = true;
				}
			}
			periodStart = cal.previousPeriodEndDate(chargeDateStart);
			periodStart = DateUtils.setTimeToZero(periodStart);
			periodEnd = cal.nextPeriodStartDate(chargeDateStart);
			periodEnd = DateUtils.setTimeToZero(periodEnd);
		}

		chargeInstance.setChargeDate(chargeDateStart);
		chargeInstance.getRecurringChargeTemplate().getCalendar()
				.setInitDate(chargeInstance.getServiceInstance().getSubscriptionDate());
		if (isLastChargingApplied) {
			chargeInstance.setNextChargeDate(null);
			chargeInstance.setStatus(InstanceStatusEnum.CLOSED);
		} else {
			chargeInstance.setNextChargeDate(
					chargeInstance.getRecurringChargeTemplate().getCalendar().nextCalendarDate(chargeDateStart));
		}
	...
	// public void
	// applyNotAppliedinAdvanceReccuringCharge(RecurringChargeInstance
	// chargeInstance, boolean reimbursement, RecurringChargeTemplate
	...
	// if (reimbursement) {
	// applicationDate =
	// cal.nextCalendarDate(chargeInstance.getTerminationDate());
	// }
	//
	// if (applicationDate == null) {
	// throw new IncorrectChargeInstanceException("ChargeDate is null.");
	...
	// if (invoiceSubCategory == null) {
	// throw new IncorrectChargeTemplateException("InvoiceSubCategory is null
	// for chargeTemplate code=" + recurringChargeTemplate.getCode());
	// }
	//
	// TradingCurrency currency = chargeInstance.getCurrency();
	// if (currency == null) {
	...
	// if (tax == null) {
	// tax =
	// invoiceSubCategoryService.evaluateTaxCodeEL(invoiceSubcategoryCountry.getTaxCodeEL(),
	// chargeInstance.getUserAccount(),
	// chargeInstance.getUserAccount().getBillingAccount(), null);
	// if (tax == null) {
	// throw new IncorrectChargeTemplateException("Tax is null for
	// invoiceSubCategoryCountry id=" + invoiceSubcategoryCountry.getId());
	// }
	// }
	//
	...
	//// Date nextapplicationDate = cal.nextCalendarDate(applicationDate); --
	// guven
	// Date nextapplicationDate = cal.nextPeriodStartDate(applicationDate);
	// log.debug("ApplyNotAppliedinAdvanceReccuringCharge next step for {},
	// applicationDate={}, nextApplicationDate={},nextApplicationDate={}",
	// chargeInstance.getId(),
	// applicationDate, nextapplicationDate, nextChargeDate);
	//
	// //Date previousapplicationDate =
	// cal.previousCalendarDate(applicationDate); -- guven
	// Date previousapplicationDate =
	// cal.previousPeriodEndDate(applicationDate);
	// previousapplicationDate =
	// DateUtils.setTimeToZero(previousapplicationDate);
	// log.debug("ApplyNotAppliedinAdvanceReccuringCharge applicationDate={},
	// nextapplicationDate={},previousapplicationDate={}", applicationDate,
	// nextapplicationDate,
	// previousapplicationDate);
	//
	// BigDecimal inputQuantity = chargeInstance.getQuantity();
	...
	// // charges echues
	// log.debug("ApplyNotAppliedinAdvanceReccuringCharge
	...
	// calendarJoin.setJoinType(CalendarJoinTypeEnum.INTERSECT);
	//
	// Date periodEnd = calendarJoin.nextCalendarDate(applicationDate);
	//
	// double part1 = DateUtils.daysBetween(periodStart, periodEnd);
	// double part2 = DateUtils.daysBetween(previousapplicationDate,
	// nextapplicationDate);
	//
	// nextapplicationDate = periodEnd; // guven
	//
	// if (part2 > 0) {
	// prorataRatio = part1 / part2;
	// } else {
	// log.error("ApplyNotAppliedinAdvanceReccuringCharge Error in calendar
	// dates : nextapplicationDate={}, previousapplicationDate={}",
	// nextapplicationDate,
	// previousapplicationDate);
	// }
	// inputQuantity = inputQuantity.multiply(new BigDecimal(prorataRatio +
	// ""));
	// log.debug("part1={}, part2={}, prorataRatio={} -> quantity", part1,
	// part2, prorataRatio, inputQuantity);
	// }
	//
	...
	//
	// log.debug("ApplyNotAppliedinAdvanceReccuringCharge :
	// nextapplicationDate={}, param2={}", nextapplicationDate, param2);
	//
	// if (reimbursement) {
	// inputQuantity = inputQuantity.negate();
	// }
	//
	// log.debug("Before walletOperation:" + (System.currentTimeMillis() -
	// startDate));
	//
	// WalletOperation walletOperation =
	// chargeApplicationRatingService.rateChargeApplication(chargeInstance,
	// reimbursement ? ApplicationTypeEnum.PRORATA_TERMINATION :
	// applicationTypeEnum, nextapplicationDate,
	// chargeInstance.getAmountWithoutTax(),
	// chargeInstance.getAmountWithTax(), inputQuantity, null, currency,
	// countryId, tax.getPercent(), null, nextapplicationDate,
	// invoiceSubCategory,
	// chargeInstance.getCriteria1(), chargeInstance.getCriteria2(),
	// chargeInstance.getCriteria3(), chargeInstance.getOrderNumber(),
	// applicationDate,
	// DateUtils.addDaysToDate(nextapplicationDate, -1), reimbursement ?
	// ChargeApplicationModeEnum.REIMBURSMENT :
	// ChargeApplicationModeEnum.SUBSCRIPTION, false, false);
	//
	// log.debug("After walletOperation:" + (System.currentTimeMillis() -
	// startDate));
	//
	// walletOperation.setSubscriptionDate(chargeInstance.getServiceInstance().getSubscriptionDate());
	//
	// List<WalletOperation> oprations = chargeWalletOperation(walletOperation);
	//
	// log.debug("After chargeWalletOperation:" + (System.currentTimeMillis() -
	// startDate));
	//
	...
	public void applyChargeAgreement(RecurringChargeInstance chargeInstance,
			RecurringChargeTemplate recurringChargeTemplate, Date endAgreementDate) throws BusinessException {

		// we apply the charge at its nextChargeDate if applied in advance, else
		// at chargeDate
		Date applicationDate = chargeInstance.getNextChargeDate();
		...
		while (applicationDate.getTime() < endAgreementDate.getTime()) {
			...
			WalletOperation chargeApplication = chargeApplicationRatingService.rateChargeApplication(chargeInstance,
					...
					inputQuantity, null, currency, countryId, tax.getPercent(), null, nextapplicationDate,
					invoiceSubCategory, chargeInstance.getCriteria1(), chargeInstance.getCriteria2(),
					chargeInstance.getCriteria3(), chargeInstance.getOrderNumber(), applicationDate, endDate,
	...
	public List<WalletOperation> listToInvoice(Date invoicingDate) {
		...
		try {
			walletOperations = getEntityManager().createNamedQuery("WalletOperation.listToInvoice")
					.setParameter("invoicingDate", invoicingDate).getResultList();
		} catch (Exception e) {
	...
	public List<WalletOperation> listToInvoiceByUserAccount(Date invoicingDate, UserAccount userAccount) {
		List<WalletOperation> walletOperations = null;
		try {
			walletOperations = getEntityManager().createNamedQuery("WalletOperation.listToInvoiceByUA")
					.setParameter("invoicingDate", invoicingDate).setParameter("userAccount", userAccount)
					.getResultList();
		} catch (Exception e) {
	...
	public List<Long> listToInvoiceIds(Date invoicingDate) {
		List<Long> ids = null;
		try {
			ids = getEntityManager().createNamedQuery("WalletOperation.listToInvoiceIds")
					.setParameter("invoicingDate", invoicingDate).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private List<WalletOperation> chargeOnWalletIds(List<Long> walletIds, WalletOperation op) throws BusinessException {
		...
		if (remainingAmountToCharge.compareTo(totalBalance) > 0
				&& walletCacheContainerProvider.isReservedBalanceCached(walletIds.get(walletIds.size() - 1))) {
	...
	public List<WalletOperation> chargeWalletOperation(WalletOperation op) throws BusinessException {
		...
		} else if (chargeInstance.getPrepaid() && (chargeInstance instanceof RecurringChargeInstance
				|| chargeInstance instanceof OneShotChargeInstance)) {
	...
	public int updateToRerate(List<Long> walletIdList) {
		...
		List<Long> ratedTransactionsBilled = (List<Long>) getEntityManager()
				.createNamedQuery("RatedTransaction.getRatedTransactionsBilled")
	...
	public List<Long> listToRerate() {
		return (List<Long>) getEntityManager()
				.createQuery("SELECT o.id FROM WalletOperation o "
	...
	@SuppressWarnings("unchecked")
	public List<WalletOperation> findWalletOperation(WalletOperationStatusEnum status, WalletTemplate walletTemplate,
			WalletInstance walletInstance, UserAccount userAccount, List<String> fetchFields, int maxResult) {
		try {
	...
	public List<WalletOperation> openWalletOperationsBySubCat(WalletInstance walletInstance,
			InvoiceSubCategory invoiceSubCategory) {
	...
	@SuppressWarnings("unchecked")
	public List<WalletOperation> openWalletOperationsBySubCat(WalletInstance walletInstance,
			InvoiceSubCategory invoiceSubCategory, Date from, Date to) {
	...
	public List<Object[]> openWalletOperationsByCharge(WalletInstance walletInstance) {
		...
		try {
			...
			List<Object[]> resultList = getEntityManager().createNativeQuery(
					"select op.description ,sum(op.quantity) QT, sum(op.amount_without_tax) MT ,op.input_unit_description from "
							+ "billing_wallet_operation op , cat_charge_template ct, billing_charge_instance ci "
							+ "where op.wallet_id = " + walletInstance.getId()
